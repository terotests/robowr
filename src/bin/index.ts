...
import * as R from "../writer/";
import { fail } from "assert";
import * as fs from "fs";
import * as path from "path";

async function run_writer() {
  ...
  if (argv.f) {
    try {
      ...
      const outputFile =
        argv.o || dirNameInput + "/gen_" + fileNameInput + ".ts";
  ...
  const LegacyInterface = async () => {
    ...
    const normalizePath = (item) => {
      return path.normalize(
        targetDir + "/" + (item.path || item.path_name) + "/" + item.name.trim()
      );
    };
    ...
    find_commands(
      path.normalize(process.cwd() + "/" + outputDir + "/.robowr/cmds/")
    );
    if (argv.a) {
      commands.forEach((c) => {
    ...
    const readCommandData = (CmdName: string): any => {
      ...
      for (let ext of extensions) {
        ...
        try {
          const TryData = fs.readFileSync(
            process.cwd() + "/.robowr/data/" + CmdName + "." + ext,
            "utf8"
          );
    ...
    const save_data = async () => {
      ...
      for (let command of commands) {
        ...
        const cmd_data_wr = wr.getFileWriter(
          ".robowr/data/",
          command.name + ".json"
        );
      ...
      try {
        prevWriteHistory = JSON.parse(
          fs.readFileSync(targetDir + "/.robowr/writes/history.json", "utf8")
        );
      } catch (e) {}
