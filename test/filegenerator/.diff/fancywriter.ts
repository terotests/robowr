const fancyFileWriterGenerator = () => [
  ...
  `}`,
  "",
  `export type generatorFunction<T extends hasWriter> = (x: T) => FancyCodeBlock<T>;`,
  `export type FancyCodeBlock<T extends hasWriter> =`,
  [
    [
      `| Array<FancyCodeBlock<T>>`,
      `| generatorFunction<T>`,
      `| undefined`,
      `| void`,
      `| string;`
  ...
  "",
  `export function Join<T extends hasWriter>(list: FancyCodeBlock<T>): CodeBlock<T> {`,
  [
    [
      `return (x: T) => {`,
      [
    ...
    [
      ...
      [
        [
          `const initialTabIndex = calculateTabIndex();`,
          `currentTabIndex = initialTabIndex;`,
          `const result: FancyCodeBlock<any> = [];`,
          `while (outputIndex < lines.length && initialTabIndex <= currentTabIndex) {`,
          [
            [
  ...
  [
    [
      ...
      `}`,
      `write(code: FancyCodeBlock<Ctx<T>>) {`,
  ...
  `*/`,
  `export function Walk<T extends hasWriter>(ctx: T, lines: FancyCodeBlock<T>) {`,
  [
    [
      `if (!ctx.writer) {`,
      [[`return ctx;`]],
  ...
  [
    [
      ...
      [
        [
          `const unwrap = (wrappedList: FancyCodeBlock<T>) => {`,
          [
            [
  ...
  [
    [
      `walk<T>(code: FancyCodeBlock<Ctx<T>>, ctx?: Ctx<T>) {`,
      [
        [
          `if (ctx) {`,
